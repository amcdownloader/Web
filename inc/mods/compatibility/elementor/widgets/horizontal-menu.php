...
class Horizontal_Menu extends The7_Elementor_Widget_Base {
	...
	protected function render() {
		...
		add_filter( 'presscore_nav_menu_link_before', [ $this, 'menu_link_before_filter' ], 10, 2 );
		add_filter( 'dt_get_thumb_img-output', [ $this, 'inline_svg_menu_filter' ], 10, 2 );
		...
		remove_filter( 'dt_get_thumb_img-output', [ $this, 'inline_svg_menu_filter' ], 10 );
		remove_filter( 'presscore_nav_menu_link_before', [ $this, 'menu_link_before_filter' ], 10 );
	 ...
	 * @param array  $config     Image config.
	 *
	 * @return string
	 */
	public function inline_svg_menu_filter( $image_html, $config ) {
		if ( isset( $config['img_id'] ) && ! empty( $config['use_elementor_svg'] ) && 'image/svg+xml' === get_post_mime_type( $config['img_id'] ) ) {
			$inline_svg = Svg::get_inline_svg( $config['img_id'] );
			if ( $inline_svg ) {
				return $inline_svg;
			}
		}
	 ...
	 * @return string
	 */
	public function menu_link_before_filter( $link_before, $item ) {
		if ( $item->description ) {
