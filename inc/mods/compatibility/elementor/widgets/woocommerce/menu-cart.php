...
class Menu_Cart extends The7_Elementor_Widget_Base {
	...
	protected function print_popup_url( $popup_id ) {
		...
		echo '<a ' . $this->get_render_attribute_string( 'popup-button' ) . '></a>';
	}
	...
	private function add_icon_sticky_controls( $prefix = '' ) {
		switch ( $prefix ) {
			...
			case '_full':
				$selectors = [
					self::STICKY_WRAPPER . ' .dt-menu-cart__toggle:not(.dt-empty-cart) .elementor-icon',
				];
				break;
			default:
				$selectors = [
	...
	private function add_indicator_sticky_controls( $prefix = '' ) {
		switch ( $prefix ) {
			...
			case '_full':
				$selectors = [
					self::STICKY_WRAPPER . '.dt-menu-cart--items-indicator-bubble .dt-menu-cart__toggle:not(.dt-empty-cart)',
				];
				break;
			default:
				$selectors = [
