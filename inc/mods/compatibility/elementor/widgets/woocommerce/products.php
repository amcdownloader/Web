...
class Products extends The7_Elementor_Widget_Base {
	...
	protected function render_add_to_cart_on_image_button( $product ) {
		...
		$this->add_render_attribute( 'button', 'class', 'woo-popup-button' );

		$button_text = $this->get_product_add_to_cart_icon_html( $product, 'popup-icon' );
	...
	protected function get_button_icon_html( $icon, $class = '' ) {
		 ...
		 */
		if ( ( empty( $icon['library'] ) || $icon['library'] === 'svg' ) && $this->get_settings_for_display( 'layout' ) === 'btn_on_img' ) {
			$icon_html = '<span class="svg-background">' . $icon_html . '</span>';
		}
	...
	protected function register_controls() {
		...
		$this->template( Add_To_Cart_Button::class )->add_style_controls(
			...
			[
				...
				'button_icon_position' => $button_icon_conditions,
				'button_icon_spacing'  => $button_icon_conditions,
			]
		);
	...
	protected function add_icon_on_image_style_controls() {
		...
		$this->add_basic_responsive_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}}' => '--on-image-icon-size: {{SIZE}}{{UNIT}}',
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons i:before' => 'font-size: var(--on-image-icon-size)',
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons svg' => 'width: var(--on-image-icon-size); height: var(--on-image-icon-size)',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors'  => [
					...
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons a .svg-background' => 'border-radius: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons svg' => 'fill: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons a .svg-background'       => 'background: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons:hover svg' => 'fill: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons:hover a .svg-background'       => 'background: {{VALUE}};',
					'{{WRAPPER}} .woo-buttons-on-img .woo-buttons a:focus'   => 'background: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors'    => [
					...
					'{{WRAPPER}}.icon-position-left .woo-buttons a .svg-background' => 'right: unset; left: 0px;',
