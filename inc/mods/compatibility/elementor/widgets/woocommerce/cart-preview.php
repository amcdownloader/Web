...
class Cart_Preview extends The7_Elementor_Widget_Base {
	...
	protected function add_filled_cart_controls() {
		...
		$this->add_control( 'footer_position', [
			'label'                => esc_html__( 'Subtotal & Buttons Position', 'the7mk2' ),
			'type'                 => Controls_Manager::SELECT,
			'default'              => 'bottom',
			'options'              => [
				'below'  => esc_html__( 'Below Products', 'the7mk2' ),
				'bottom' => esc_html__( 'Bottom', 'the7mk2' ),
				'stick_bottom' => esc_html__( 'Stick To Bottom', 'the7mk2' ),
			],
			'selectors'            => [
				'{{WRAPPER}} .the7-e-woo-cart-wrapper .the7-e-woo-cart-content' => '{{VALUE}}',
			],
			'selectors_dictionary' => [
				'bottom' => 'flex: 1 1 auto;',
				'stick_bottom' => 'flex: 1 1 0; overflow-y: auto; -webkit-overflow-scrolling: touch;'
			],
            'prefix_class' => 'footer-position-',
		] );


		$this->add_control( 'widget_height_select', [
			'label'                => esc_html__( 'Min Height', 'the7mk2' ),
			'type'                 => Controls_Manager::SELECT,
			'default'              => 'fit',
			'options'              => [
				'fit'  => esc_html__( 'Fit To Screen', 'the7mk2' ),
				'custom' => esc_html__( 'Custom', 'the7mk2' ),
			],
			'condition'  => [
				'footer_position' => ['bottom', 'stick_bottom'],
			],
			'prefix_class' => 'footer-height-',
		] );

		$this->add_responsive_control( 'stick_to_bottom_widget_height', [
			'label'      => esc_html__( 'Min Height', 'the7mk2' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px', 'vh' ],
			'selectors'  => [
				'{{WRAPPER}} > .elementor-widget-container'            => 'min-height: {{SIZE}}{{UNIT}}',
				'.admin-bar {{WRAPPER}} > .elementor-widget-container' => 'min-height: calc({{SIZE}}{{UNIT}} - 32px)',
			],
			'default'    => [
				'unit' => 'vh',
				'size' => '100',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', [ 'the7-e-woo-cart-wrapper', $cart_status ] );
